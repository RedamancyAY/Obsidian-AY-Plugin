...
ROOT_PATHs = NameCfgNode(
    WaveFake="/usr/local/ay_data/dataset/0-deepfake/WaveFake",
    LibriSeVoc="/usr/local/ay_data/dataset/0-audio/LibriSeVoc",
    DECRO="/usr/local/ay_data/dataset/0-audio/DECRO",
)
...
def WaveFake(task="inner_eval"):
    ...
    C.root_path = "/usr/local/ay_data/dataset/0-deepfake/WaveFake"
    C.task = task
    if task == "inner_eval":
        task = WaveTasks[task]
...
def LAVDF(task=None):
    C = ConfigurationNode()
    C.root_path = "/home/ay/data/0-原始数据集/LAV-DF-Audio"
    return C


# ### ADD2023

def ADD2023(task=None):
    C = ConfigurationNode()
    C.root_path = "/home/ay/data/ADD2023"
    return C
