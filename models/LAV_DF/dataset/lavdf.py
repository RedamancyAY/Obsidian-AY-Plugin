...
class Lavdf(Dataset):
    def __init__(
        ...
        if metadata is None:
            metadata: List[Metadata] = read_json(
                os.path.join(self.root, "metadata.min.json"), lambda x: Metadata(**x)
            )
            self.metadata: List[Metadata] = [
                each for each in metadata if each.split == subset
            ]
        ...
        if self.require_match_scores:
            ...
            # [-0.05, ..., 0.985]
            self.anchor_x_min = [
                temporal_gap * (i - 0.5) for i in range(self.video_padding)
            ]
            # [0.05, ..., 0.995]
            self.anchor_x_max = [
                temporal_gap * (i + 0.5) for i in range(self.video_padding)
    ...
    def __getitem__(self, index: int) -> List[Tensor]:
        ...
        if not self.require_match_scores:
            ...
            return [video, audio, *label] + self.get_meta_attr(
                meta, video, audio, label
            )
    ...
    def get_label_with_match_scores(
        self, meta: Metadata
        ...
        boundary_map_file_path = os.path.join(
            self.root, "label", boundary_map_file_name
        )

        match_scores_file_name = meta.file.split("/")[-1].split(".")[0] + ".npz"
        match_scores_file_path = os.path.join(
            self.root, "match_scores", match_scores_file_name
        )

        if os.path.exists(boundary_map_file_path) and os.path.exists(
    ...
    def _get_train_label(self, frames, video_labels, temporal_scale, fps=25) -> T_LABEL:
        ...
        ##########################################################################################################
        gt_iou_map = F.pad(
            gt_iou_map.float(), pad=[0, self.video_padding - frames, 0, 0]
        )
        ...
        if len(gt_start_bboxs) > 0:
            ...
            for jdx in range(len(self.anchor_x_min)):
                match_score_start.append(
                    np.max(
                        ioa_with_anchors(
                            self.anchor_x_min[jdx],
                            self.anchor_x_max[jdx],
                            gt_start_bboxs[:, 0],
                            gt_start_bboxs[:, 1],
                        )
                    )
...
def _default_get_meta_attr(
    meta: Metadata, video: Tensor, audio: Tensor, label: Tensor
) -> List[Any]:
...
class LavdfDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        get_meta_attr: Callable[
            [Metadata, Tensor, Tensor, Tensor], List[Any]
        ] = _default_get_meta_attr,
    ):
        super().__init__()
        self.root = root
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        self.train_dataset = self.Dataset(
            ...
            get_meta_attr=self.get_meta_attr,
            require_match_scores=self.require_match_scores,
        )
        self.dev_dataset = self.Dataset(
            "dev",
            self.root,
            self.frame_padding,
            self.max_duration,
    ...
    def train_dataloader(self) -> TRAIN_DATALOADERS:
        return DataLoader(
            ...
            sampler=RandomSampler(
                self.train_dataset, num_samples=self.take_train, replacement=True
            ),
        )
...
feature_type_to_dataset_type = {(None, None): Lavdf}
