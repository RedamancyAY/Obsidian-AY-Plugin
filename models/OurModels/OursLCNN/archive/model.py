...
lcnn.extract_feature(x, debug=True).shape


class OursLCNN(nn.Module):
    def __init__(self, n_LFCC=80, use_attn_pool=False):
        super().__init__()
        ...
        if use_attn_pool:
            self.pool = AttentiveStatisticsPooling1D(n_dim, n_dim * 2, 1)
            self.fc1 = nn.Linear(n_dim * 2, 512)
        else:
            self.pool = None
    ...
    def extract_feature(self, x, debug=0):
        # lfcc = self.lfcc(x) # (B, 1, 80, 301)
        # print(lfcc.shape)
        # spec_feat = rearrange(lfcc, "b c h w -> b c w h")

        # leaf = self.leaf_audio(x) # (B, 80, 300)
        # spec_feat = rearrange(leaf, 'b h w -> b 1 w h')

        spec_feat = self.extract_spec(x)
        # spec_feat = rearrange(spec_feat, 'b 1 h w -> b 1 w h')

        x = self.lcnn.extract_feature(spec_feat, debug=debug)  # (B, 32, W, n_LFCC//16)
        x = rearrange(x, "b c h w -> b c w h")

        x = self.lstm(x)  # (B, W, 32 * n_LFCC//16), (B, W, 160)
        if not self.use_attn_pool:
            x = torch.mean(x, dim=1)  # (B, 32 * n_LFCC//16)
        else:
            x = self.pool(x)  # (B, 32 * n_LFCC//16 * 2)
