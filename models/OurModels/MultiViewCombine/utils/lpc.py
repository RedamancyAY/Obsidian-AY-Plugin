...
class LPCSlicer(nn.Module):
    ...
    def forward(self: "LPCSlicer", X: torch.Tensor) -> torch.Tensor:
        ...
        """
        X = nn.functional.pad(X, (0, self.padding), "constant", 0.0)
        print(X.shape, self.size, self.offset)
...
class LPCCoefficients(nn.Module):
    ...
    def forward(self: "LPCCoefficients", X: torch.Tensor) -> torch.Tensor:
        ...
        alphas = torch.zeros(
            (B, F, self.p), dtype=X.dtype, device=X.device, requires_grad=False
        )
        alphas[:, :, 0] = 1.0
        alphas_prev = torch.zeros(
            (B, F, self.p), dtype=X.dtype, device=X.device, requires_grad=False
        )
        alphas_prev[:, :, 0] = 1.0
        ...
        den = (
            (fwd_error * fwd_error).sum(axis=-1) + (bwd_error * bwd_error).sum(axis=-1)
        ).unsqueeze(-1)
        ...
        for i in range(self.order):
            ...
            dot_bfwd = (bwd_error * fwd_error).sum(axis=-1).unsqueeze(-1)

            reflect_coeff = -2.0 * dot_bfwd / den
            alphas_prev, alphas = alphas, alphas_prev

            for j in range(1, i + 2):
                alphas[:, :, j] = (
                    alphas_prev[:, :, j]
                    + reflect_coeff[:, :, 0] * alphas_prev[:, :, i - j + 1]
                )
