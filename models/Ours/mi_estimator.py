...
class CLUBSample_group(nn.Module):  # Sampled version of the CLUB estimator
    ...
    def mi_est(
        ...
        # log of conditional probability of positive sample pairs
        mu_exp1 = mu.unsqueeze(1).expand(
            -1, y_samples.shape[1], -1
        )  # (bs, y_dim) -> (bs, T, y_dim)
        # logvar_exp1 = logvar.unqueeze(1).expand(-1, y_samples.shape[1], -1).reshape(-1, logvar.shape[-1])
        positive = (
            -((mu_exp1 - y_samples) ** 2).mean(dim=1) / logvar.exp()
        )  # mean along T
        negative = (
            -((mu_exp1 - y_samples[random_index]) ** 2).mean(dim=1) / logvar.exp()
        )  # mean along T

        return ((positive.sum(dim=-1) - negative.sum(dim=-1))**2).mean() / 2
