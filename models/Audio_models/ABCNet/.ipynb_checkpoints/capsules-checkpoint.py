...
class RoutingCapsules(nn.Module):
	def __init__(self, in_dim, in_caps, num_caps, dim_caps, num_routing, device: torch.device='cpu'):
		"""
		Initialize the layer.
	...
	def forward(self, x):
		...
		'''
		b = torch.zeros(batch_size, self.num_caps, self.in_caps, 1).to(x.device)
