...
def display_ASV2019LA_evaluation(models, versions=[], show_latex=0, metric_prefix="test", file_name="test"):
    ...
    for model, version in zip(models, versions):
        _data = read_test_result(model, task, version, metric_prefix=metric_prefix, file_name=file_name)
        if _data is None:
            continue
        ...
        # print(model, version, task, len(res), METHODs)
        res["method"] = METHODs
        DATA.append(res)
...
class BaseDisplay:
    def __init__(self, models, versions=[], task=None, calc_avg=True, metric_prefix="test"):
    ...
    def read_data_for_model(self, model, version, model_name, calc_avg=True):
        ...
        _data = read_test_result(model, self.task, version, metric_prefix=self.metric_prefix)
        if _data is None:
            return None
        ...
        if calc_avg:
            res.loc["Avg", :] = res.mean()
        res["res"] = res.apply(
            lambda x: format_res(x, acc=0, auc=1, eer=1, metric_prefix=self.metric_prefix),
            axis=1,
        )
    ...
    def display(self, show_latex=0):
        ...
        if show_latex:
            print(self.data.style.to_latex(column_format="lrr"))
...
def display_ASV2021_evaluation(models, versions=[], show_latex=0, metric_prefix="test"):
    metric_auc = f"{metric_prefix}-auc"
    metric_eer = f"{metric_prefix}-eer"
    METHODs = [
        "inner",
...
def display_ASV2021_evaluation_versions(model, versions=[], show_latex=0, metric_prefix="test"):
    metric_auc = f"{metric_prefix}-auc"
    metric_eer = f"{metric_prefix}-eer"
    METHODs = [
        "inner",
